...
library(dplyr)
library(Hmisc)
Greenville <- read_excel("~/Desktop/Greenville.xlsx")
Wesleyen <- read_excel("~/Desktop/Illinois Wesleyan.xlsx")
iwID <- read_excel("~/Desktop/Illinois Wesleyan.xlsx", sheet=2)
gID <- read_excel("~/Desktop/Greenville.xlsx", sheet=2)
Case <- read_excel("~/Desktop/CWRU.xlsx")
Cid <- read_excel("~/Desktop/CWRU.xlsx", sheet=2)


merge2 <- merge(iwID, Wesleyen)
total <- rbind(merge, merge2)
##mintz <- total[which(total$pitcher=='mintz'),]
##batter <- merge[which(merge$batter==unique(merge$batter)[1]),]
info <- Case
IDs <- Cid
batter <- 'capetillo'
pitcher <- 'margolin'
